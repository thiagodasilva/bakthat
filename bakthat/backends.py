...
class GlusterBackend(BakthatBackend):
    ...
    def download(self, keyname):
        ...
        """
        self.startupVolume()
        dst_file = tempfile.TemporaryFile()
        ascii_keyname = keyname.encode('ascii', 'ignore')
        src_filename = "%s/%s" % (self.backup_dir, ascii_keyname)
        try:
            with self.vol.open(src_filename, os.O_RDONLY) as src_file:
                data = src_file.read(self.read_size)
                while data:
                    dst_file.write(data)
                    data = src_file.read(self.read_size)
        except OSError as ose:
            log.error("Error reading from file %s: %s" % 
                    (src_filename, ose.strerror))
            return None
